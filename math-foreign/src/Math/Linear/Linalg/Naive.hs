-----------------------------------------------------------
-- |
-- module:                      Math.Linear.Linalg.Naive
...
module Math.Linear.Linalg.Naive
  ( module Math.Linear.Matrix.Naive
   -- * Matrix Properties
  , det
  , trace
  , rank
...
import qualified Math.Linear.Matrix.Mutable.Naive as Mutable
import Math.Linear.Matrix.Naive (Mat(..), unsafeWith, unsafeOp, unsafeUnaryOp, unsafeFactorizeOp, unsafeDecomposeOp)
