...
import Foundation
import Founaation.Foreign
import Founaation.Primitive
...
import Math.Linear.Matrix
       (Mat(..), unsafeDecomposeOp, unsafeOp, unsafeUnaryOp)
...
transform
    :: I.Elem a
    => Mat a -> V.Vector a -> V.Vector a
transform m@M {..} v =
    unsafePerformIO $
    do v' <- MV.new row
       MV.unsafeWith v' $
           \xr ->
                M.unsafeWith m $ \xs r c -> V.unsafeWith v $ \xv -> I.call $ I.transform xr r c xs xv
       V.unsafeFreeze v'
